    ...
    public static void main(final String[] args) {
        ...
        for(int round = 1; round <= no_rounds; round++){
            ...
            for(int sub_round = 1; sub_round <= no_players; sub_round++){
                System.out.println("SUBRUNDA " + sub_round);
                sheriff = players.get(sub_round - 1);
                initgold = sheriff.getGold();
                System.out.println("SHERIFF GOLD = " + sheriff.getGold());
                for(int i = 1; i <= no_players; i++){
                    ...
                    System.out.println("Banii lui " + i + " inainte de inspectie = " + p.getGold());
                    System.out.println("Cartile lui " + i);
                    p.printHand();
                    p.createSack(round);     // Creeaza sacul
                    System.out.println("Sacul lui " + i);
                    System.out.println(p.getSack());
                    // Verificarea sacului si adaugarea cartilor confiscate in deck
                    deck.addAll(sheriff.verify(p, initgold));
                    System.out.println("Banii lui " + i + " dupa de inspectie = " + p.getGold());
                    System.out.println("Taraba lui" + i);
                    System.out.println(p.getStall());
